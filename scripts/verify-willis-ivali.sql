...
SELECT
  ...
  -- Active visits aligned with schema (see hospital-database.sql)
  (SELECT COUNT(*) FROM patient_visits pv JOIN patients p ON p.id = pv.patient_id
    WHERE p.email = 'itsivali@gmail.com'
    AND pv.status IN ('checked_in', 'in_consultation', 'in_treatment', 'awaiting_lab', 'awaiting_pharmacy', 'admitted')) as active_visits,
